...
def process_fort38(input_file: str, output_file: str, log_file: str):
    ...
    with open(output_file, 'w') as f:
        # Write header: ' ph' + 12 spaces + pH values (each 5 chars right-aligned + 1 space)
        f.write(' ph            ')  # Exactly 15 characters (14 + 1)
        f.write(''.join(f'{ph:>5.1f} ' for ph in ph_values))
        f.write('\n')

        # Write conformer data: name (14 chars) + space + probabilities (each 5 chars + 1 space)
        for conf in all_conformers:
...
def generate_sum_crg(conformers: List[Conformer], ph_values: List[float], 
    ...
    for res_id, res_conformers in residue_groups.items():
        ...
        for sign in ['+', '-']:  # Only positive and negative
    ...
    def sort_key(identifier):
        # Extract chain and resnum
        # Format: RESTYPE(3) + SIGN(1) + CHAIN(1) + RESNUM(4) + '_'
        chain = identifier[-6]
    ...
    # Calculate net charges (sum of all residue charges)
    ...
    # Write output file
    with open(output_file, 'w') as f:
        # Write header - match sum_crg.out format
        f.write(' ph            ')  # 15 characters
        f.write(''.join(f'{ph:>5.1f} ' for ph in ph_values))
        f.write('\n')
        
        # Write residue data
        for res_id in sorted_residues:
            # Pad identifier to 15 characters
            f.write(f'{res_id:<15}')
            # Write charges with 2 decimal places, 5 characters wide
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        ...
        print("Usage: python xts_HETATMcorr.py [fort.38]")
        ...
        print("  - Generate xts_sum_crg.out (corrected charge summary)")
    ...
    try:
        conformers, ph_values, residue_groups = process_fort38(input_file, output_file, log_file)
