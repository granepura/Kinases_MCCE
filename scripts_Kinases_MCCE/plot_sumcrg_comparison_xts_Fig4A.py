...
# === Configuration (trial-aware) ===
# The trial is this script's own directory, so TrialNN/3-...py needs no paths.
# Every output carries the trial name so figures, tables and CSVs stay traceable
# once they are copied out of the trial.
import argparse
import csv

_ap = argparse.ArgumentParser(
    description="Fig4A: per-residue charge, holo (run_holo) vs apo (run_apo), "
                "for one trial.")
...
plot_dir = os.path.join(TRIAL, "plots_Fig4A_holo_vs_apo_xts")
log_file = os.path.join(plot_dir, f"{TRIAL_NAME}_Fig4A_holo_vs_apo.log")
...
THRESHOLD_EPS = 1e-9
outlier_table_file = f"{TRIAL_NAME}_Fig4A_outliers_holo_vs_apo.tsv"
...
print_log("="*80)
print_log(f"Fig4A  holo vs apo charge  |  TRIAL: {TRIAL_NAME}")
print_log("="*80)
...
for idx, (pdb, (inhibitor, icode)) in enumerate(sorted(inhibitor_codes.items()), 1):
    ...
    for key in shared_keys:
        ...
        points_all.append({
            'trial': TRIAL_NAME, 'pdb': pdb, 'inhibitor': inhibitor, 'key': key,
    ...
    outpath = os.path.join(plot_dir, f"{TRIAL_NAME}_Fig4A_{pdb}_{inhibitor}.png")
...
if all_x_all and all_y_all:
    ...
    all_out = os.path.join(plot_dir, f"{TRIAL_NAME}_Fig4A_ALL_{y_label}_vs_{x_label}.png")
...
points_csv = os.path.join(plot_dir, f"{TRIAL_NAME}_Fig4A_holo_vs_apo_points.csv")
with open(points_csv, "w", newline="") as fh:
    _cols = ["trial", "pdb", "inhibitor", "key", "resname", "chain", "resnum",
             "label", "is_inhibitor", "apo", "holo", "delta"]
    w = csv.DictWriter(fh, fieldnames=_cols)
    w.writeheader()
    for rec in points_all:
        w.writerow({k: (f"{rec[k]:.6f}" if k in ("apo", "holo", "delta") else rec[k])
                    for k in _cols})
print_log(f"  Per-point CSV written to:    {points_csv}  ({len(points_all)} rows)")
