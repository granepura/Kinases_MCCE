...
_methods = os.path.join(plot_dir, f"{LABEL}_Fig4A_METHODS.md")
_repro = os.path.join(plot_dir, f"{LABEL}_Fig4A_reproducibility.tsv")
write_methods(_methods, _repro, "Fig4A (per-residue charge, holo vs apo)",
              TRIALS, ROOT, {f"{k[0]}/{k[1]}": AGG[k]["delta"] for k in AGG}, _args.err)
print_log(f"  Methods record written to:   {_methods}")
