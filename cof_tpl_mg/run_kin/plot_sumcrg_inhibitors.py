#!/usr/bin/env python
"""
Aggregate inhibitor scatter plot: sum_crg.out dir1 vs dir2
...
# === Config ===
dir1 =     "/home/granepura/Mar11/5-Kinases/final/cof_tpl_mg/run_cof2"
dir2 =     "/home/granepura/Mar11/5-Kinases/final/cof_tpl_mg/run_kin"
lst_file = "/home/granepura/Mar11/5-Kinases/final/cof_tpl_mg/pdb_inhibitor.lst"
plot_dir = "plots_inhibitors_kin_vs_cof2"
log_file = "plot_sumcrg_inhibitors.log"
...
for idx, (pdb, (inhibitor, icode)) in enumerate(sorted(inhibitor_map.items()), 1):
    ...
    f1 = os.path.join(dir1, pdb, "sum_crg.out")
    f2 = os.path.join(dir2, pdb, "sum_crg.out")
    ...
    if not f1_exists:
        print_log(f"      ├─ ❌ Missing: {x_label}/sum_crg.out")
    if not f2_exists:
        print_log(f"      ├─ ❌ Missing: {y_label}/sum_crg.out")
...
# Labels & styling
ax.set_xlabel("Boltzmann Weighted Solution Charge", fontweight="bold", fontsize=11)
ax.set_ylabel("Boltzmann Weighted Bound Charge", fontweight="bold", fontsize=11)
