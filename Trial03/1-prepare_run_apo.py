...
Name: 1-prepare_run_apo.py   (canonical copy: scripts_Kinases_MCCE/prepare_run_apo.py)
...
For every per-structure directory in run_holo that holo's stepB has split:
  ...
  3. link step2_out.pdb -> apo_step2_out.pdb, resetting the link if one is
     ...
     nothing else,
  4. write provenance.txt with the source path and the sha256 of both variants,
     so a stale apo is detectable later.
...
NOTE: the copy includes holo's submit_mcce4_s1s2.sh.  pro_batch overwrites
submit_mcce4_s3s4.sh when you launch apo, but that s1s2 script stays, and
running it inside an apo directory would re-run step1/step2 there and destroy
the shared-conformer premise.  Do not run it; provenance.txt repeats the warning.
...
USAGE:
======
  cd TrialNN && ./1-prepare_run_apo.py            # seed every structure
                ./1-prepare_run_apo.py --dry-run  # report only, write nothing
                ./1-prepare_run_apo.py --keep     # skip existing dirs instead
                                                  #   of replacing them
                ./1-prepare_run_apo.py 1XKK 2ITZ  # just these structures
...
def find_trial(explicit):
    """The trial directory: -t, else the directory holding this script."""
    if explicit:
    ...
    if any((here / n).is_dir() for n in SRC_NAMES):
        return here
    sys.exit(f"ERROR: {here} holds no {' or '.join(SRC_NAMES)}/ -- run this from "
             f"inside a trial directory, or pass -t /path/to/TrialNN.")
...
def main():
    ...
    ap.add_argument("-o", "--out", default="run_apo", help="destination (default: run_apo)")
    ...
    print(f"{CYAN}1-prepare_run_apo.py{RESET}")
    print(f"  trial : {trial}")
    ...
    for src in sources:
        ...
        digest, apo_digest = sha256(holo_v), sha256(apo_v)
        replacing = " (replacing existing)" if dst.exists() else ""

        if args.dry_run:
            print(f"{GREEN}[WOULD]{RESET}    {pid}: copy {src_root.name} -> "
                  f"{args.out}{replacing}, holo {digest[:12]} + apo "
                  f"{apo_digest[:12]}")
            ok += 1
            continue
        ...
        if left:
            print(f"{RED}[FATAL]    {pid}: still holds {', '.join(left)} after the "
                  f"purge -- removing {args.out}/{pid}{RESET}")
            shutil.rmtree(dst)
            failed += 1
            continue
        ...
        (dst / "provenance.txt").write_text(
            f"""prepared_by  : 1-prepare_run_apo.py
prepared_at  : {datetime.now():%Y-%m-%d %H:%M:%S}
source_dir   : {src}
holo_sha256  : {digest}   (holo_step2_out.pdb)
apo_sha256   : {apo_digest}   (apo_step2_out.pdb)
...
note         : apo reuses holo's step2 conformers.  step1/step2 are OFF in
               ...
               the ligand's electrostatic contribution.  Re-prepare if the
               sha256 values no longer match {src}.
WARNING      : holo's submit_mcce4_s1s2.sh was copied in with everything else.
               Do NOT run it here -- it would re-run step1/step2 in this apo
               directory and destroy the shared-conformer premise.
""")

        print(f"{GREEN}[PREPARED]{RESET} {pid}: holo {digest[:12]} + apo "
              f"{apo_digest[:12]}{replacing}")
        ok += 1
